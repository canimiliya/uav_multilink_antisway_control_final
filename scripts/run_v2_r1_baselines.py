"""Run the V2-R1 development-only traditional baseline freeze.
...
def summarize_trace(trace: dict, sample: dict, output_csv: Path | None) -> dict:
    ...
    mask = (pos <= 0.05) & (ori <= 5.0) & (speed <= 0.10)
    acquired = False
    acquisition = None
    ...
    result = {
        ...
        "control_effort": float(np.trapezoid(trace["ax_cmd"] ** 2, t)), "control_rate": control_rate_proxy(t, trace["ax_cmd"]),
...
def run_case(kind: str, params: dict, yz: dict, sample: dict, output_csv: Path | None = None) -> dict:
    ...
    shared_yz = (0.0, 0.0)
    trace = {key: [] for key in ("time", "position_error_3d_m", "orientation_error_deg", "tip_speed_m_s", "ax_cmd", "ay_cmd", "az_cmd", "uav_z", "tip_x", "tip_y", "tip_z", "target_x", "target_y", "target_z", "wind_x", "thrust", "torque", "joint_angles", "roll", "pitch", "anchor_active", "rotor_commands")}
    for step in range(physics_steps + 1):
        ...
        if step % outer_steps == 0:
            state = reader.read(model, data)
            shared_yz = inner.shared_yz_command(state, reference)
            if kind == "pid":
        ...
        if step % signal_steps == 0:
            ...
            jacp = np.zeros((3, model.nv)); jacr = np.zeros((3, model.nv)); mujoco.mj_jacSite(model, data, jacp, jacr, ids["tip"])
...
def write_grid(name: str, rows: list[dict]) -> None:
    (R1 / name).write_text(json.dumps({"grid": rows, "grid_size": len(rows)}, indent=2, allow_nan=False) + "\n", encoding="utf-8", newline="\n")


def write_candidates(name: str, summaries: list[dict]) -> None:
    keys = ["candidate_id", "sample_count", "safe_sample_count", "task_success_count", "safety_rate", "task_success_rate", "acquisition_median_s", "position_rmse_3d_m", "orientation_rmse_deg", "control_effort", "ramp_peak_position_error_m", "ramp_steady_state_position_error_m", "gain_norm"]
    with (R1 / name).open("w", encoding="utf-8", newline="") as stream:
...
def main() -> int:
    ...
    yz_grid = read_json("shared_yz_grid.json")
    # Separate development-only y and z scaffold tuning, using calm +/-axis samples only.
    yz_results = {"y": [], "z": []}
    zero_pid = {"candidate_id": "scaffold_probe", "kp": 0.0, "kd": 0.0, "ki": 0.0}
    for axis, candidates in (("y", yz_grid["y"]), ("z", yz_grid["z"])):
        directions = {"y": {"+y", "-y"}, "z": {"+z", "-z"}}[axis]
        samples = [row for row in development if row["scenario"] == "CALM_3D_SETPOINT" and row["target"]["direction"] in directions]
        for i, candidate in enumerate(candidates):
            ...
            yz_results[axis].append({"candidate": candidate, "score_position_rmse_m": score, "safe_count": sum(row["safe"] for row in rows), "samples": len(rows), "shared_yz": yz})
    yz_freeze = {}
    for axis in ("y", "z"):
        ranked = sorted(yz_results[axis], key=lambda row: (-row["safe_count"], row["score_position_rmse_m"], row["candidate"]["kp"] ** 2 + row["candidate"]["kd"] ** 2))
    ...
    (R1 / "shared_yz_freeze.json").write_text(json.dumps({"selection_rule": "separate axis calm +/-axis development samples", "selected": shared_yz, "grid_results": yz_results, "frozen_limits": {"ay_abs_max_m_s2": 2.0, "az_abs_max_m_s2": 2.0, "axis_slew_max_m_s2_per_update": 0.25}}, indent=2, allow_nan=False) + "\n", encoding="utf-8", newline="\n")
    ...
    for kind, selected in top3.items():
        ...
        for rank, candidate_summary in enumerate(selected, 1):
            ...
            development_rows.extend({"controller": kind, "stage2_rank": rank, **{key: row[key] for key in ("sample_id", "safe", "task_success", "acquisition_time_s", "position_rmse_3d_m", "orientation_rmse_deg", "ramp_peak_position_error_m", "ramp_steady_state_position_error_m", "control_effort", "max_abs_ax_m_s2", "max_ax_step_m_s2", "max_thrust_N", "max_abs_torque_Nm")}} for row in rows)
            print(f"stage2 {kind} rank={rank} complete", flush=True)
        stage2_summaries[kind] = completed
    selected_final = {kind: sorted(items, key=selection_key)[0] for kind, items in stage2_summaries.items()}
    for kind, summary in selected_final.items():
        ...
        (R1 / {"pid": "pid_freeze.json", "lqr": "lqr_freeze.json", "task_lqr": "task_lqr_freeze.json"}[kind]).write_text(json.dumps(freeze, indent=2, allow_nan=False) + "\n", encoding="utf-8", newline="\n")
    (R1 / "development_baseline_results.csv").write_text("", encoding="utf-8", newline="")
    if development_rows:
        keys = list(development_rows[0])
        with (R1 / "development_baseline_results.csv").open("w", encoding="utf-8", newline="") as stream:
    ...
    (R1 / "development_baseline_summary.json").write_text(json.dumps({"shared_yz": shared_yz, "final_selected": summary_payload, "stage1_top3": {kind: [{key: value for key, value in item.items() if key not in {"rows", "parameters"}} | {"parameters": item["parameters"]} for item in selected] for kind, selected in top3.items()}}, indent=2, allow_nan=False) + "\n", encoding="utf-8", newline="\n")
    ...
    (R1 / "primary_traditional_baseline.json").write_text(json.dumps({"primary_controller": primary["parameters"]["candidate_id"], "controller_family": next(kind for kind, item in selected_final.items() if item is primary), "selection_rule": "lexicographic safety, success, acquisition, 3D RMSE, orientation RMSE, ramp errors, effort", "selected_metrics": {key: value for key, value in primary.items() if key not in {"rows", "parameters"}}}, indent=2, allow_nan=False) + "\n", encoding="utf-8", newline="\n")
    (R1 / "traditional_metric_envelope.json").write_text(json.dumps(envelope, indent=2, allow_nan=False) + "\n", encoding="utf-8", newline="\n")
    advanced_contract = {"contract": "V2-R1-advanced-win-r1", "written_before_any_advanced_performance": True, "safety_rate_must_be_at_least_best_traditional": True, "task_success_rate_must_be_at_least_best_traditional": True, "position_rmse_improvement_required_fraction": 0.05, "common_success_acquisition_improvement_required_fraction": 0.05, "ramp_peak_or_steady_state_improvement_required_fraction": 0.05, "paired_bootstrap_confidence": 0.95, "paired_bootstrap_required": True, "improvement_direction": "positive means advanced metric is lower than the corresponding best traditional envelope", "holdout_only_for_final_pass": True}
    (R1 / "advanced_win_contract.json").write_text(json.dumps(advanced_contract, indent=2, allow_nan=False) + "\n", encoding="utf-8", newline="\n")
    ...
    (R1 / "safety_audit.json").write_text(json.dumps(safety_audit, indent=2, allow_nan=False) + "\n", encoding="utf-8", newline="\n")
    gate = {"task": "V2-R1-TRADITIONAL-3D-BASELINES-DEVELOPMENT-AND-FREEZE-R1", "start_head": START_HEAD, "branch": "research-v2", "main_unchanged": True, "v1_tag_unchanged": True, "shared_3d_scaffold_frozen": True, "development_target_count": 12, "holdout_diagonal_target_count": 8, "pid_grid_size": 18, "lqr_grid_size": 64, "task_lqr_grid_size": 27, "pid_selected": True, "lqr_selected": True, "task_lqr_selected": True, "primary_traditional_selected": True, "traditional_metric_envelope_written": True, "advanced_win_contract_frozen": True, "advanced_self_started": False, "advanced_paper_selected": False, "holdout_executed": False, "result": "V2_TRADITIONAL_BASELINES_FROZEN"}
    (R1 / "gate.json").write_text(json.dumps(gate, indent=2, allow_nan=False) + "\n", encoding="utf-8", newline="\n")
