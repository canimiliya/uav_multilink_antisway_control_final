...
def audit(candidate_id: str, values: list[dict], baseline: dict[str, list[dict]]) -> dict:
    ...
    acquisition_degradation = None if metric["acquisition_median_s"] is None else ev.degrade(metric["acquisition_median_s"], full["acquisition_median_s"])
    return {"candidate_id": candidate_id, "overall": metric, "strong": strong, "normal": normal, "bootstrap": boot, "gates": gates, "gate_pass_count": sum(gates.values()), "gate_count": len(gates), "final_pass": all(gates.values()), "position_improvement_vs_full": ev.improve(full["position_mean_m"], metric["position_mean_m"]), "acquisition_degradation_vs_full": acquisition_degradation}
...
def main() -> int:
    ...
    write("development_gate_audit.json", {"candidate_count": len(audits), "ranked": audits, "qualified_ids": [row["candidate_id"] for row in passes], "final_status": status, "holdout_status": holdout, "technical_retries": [{"reason": "24-worker Windows page-file exhaustion before any Stage-B cache was written", "resolution": "same commit/config/samples completed with 12 workers"}, {"reason": "finalizer initially subtracted null acquisition", "resolution": "diagnostic degradation stored as null; frozen acquisition gate remained false"}]})
